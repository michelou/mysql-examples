...
set __CJ_PREFIX=mysql-connector-j
set __CJ_JAR=

if defined CJ_HOME (
    set "_CJ_HOME=%CJ_HOME%"
    if %_DEBUG%==1 echo %_DEBUG_LABEL% Using environment variable CJ_HOME 1>&2
) else (
    set __PATH=C:\opt
    for /f "delims=" %%f in ('dir /ad /b "!__PATH!\%__CJ_PREFIX%*" 2^>NUL') do set "_CJ_HOME=!__PATH!\%%f"
    for %%i in ('where /r "!_CJ_HOME!" my*.jar 2^>NUL') do set "__CJ_JAR=%%i"
    if defined __CJ_JAR (
        if %_DEBUG%==1 echo %_DEBUG_LABEL% Using default Connector/J installation directory "!_CJ_HOME!" 1>&2
    ) else (
        echo %_ERROR_LABEL% Connector/J library not found in directory "!_CJ_HOME!" 1>&2
        set _CJ_HOME=
        set _EXITCODE=1
        goto :eof
    )
...
set __CNET_PREFIX=mysql-connector-net
set __CNET_DLL=

if defined CNET_HOME (
    set "_CNET_HOME=%CNET_HOME%"
    if %_DEBUG%==1 echo %_DEBUG_LABEL% Using environment variable CNET_HOME 1>&2
) else (
    set __PATH=C:\opt
    for /f "delims=" %%f in ('dir /ad /b "!__PATH!\%__CNET_PREFIX%*" 2^>NUL') do set "_CMET_HOME=!__PATH!\%%f"
    for %%i in ('where /r "!_CNET_HOME!" my*.dll 2^>NUL') do set "__CNET_DLL=%%i"
    if defined __CNET_DLL (
        if %_DEBUG%==1 echo %_DEBUG_LABEL% Using default Connector/NET installation directory "!_CNET_HOME!" 1>&2
    ) else (
        echo %_ERROR_LABEL% Connector/NET library not found in directory "!_CNET_HOME!" 1>&2
        set _CNET_HOME=
        set _EXITCODE=1
        goto :eof
    )
...
@rem output parameters: _MYSQL_HOME, _MYSQL_PATH
:mysql
set _MYSQL_HOME=
set _MYSQL_PATH=

set __MYSQL_CMD=
for /f "delims=" %%f in ('where mysql.exe 2^>NUL') do set "__MYSQL_CMD=%%f"
if defined __MYSQL_CMD (
    if %_DEBUG%==1 echo %_DEBUG_LABEL% Using path of MySQL executable found in PATH 1>&2
    for /f "delims=" %%i in ("%__MYSQL_CMD%") do set "__MYSQL_BIN_DIR=%%~dpi"
    for /f "delims=" %%f in ("!__MYSQL_BIN_DIR!") do set "_MYSQL_HOME=%%~dpf"
    @rem keep _MYSQL_PATH undefined since executable already in path
    goto :eof
) else if defined MYSQL_HOME (
    set "_MYSQL_HOME=%MYSQL_HOME%"
    if %_DEBUG%==1 echo %_DEBUG_LABEL% Using environment variable MYSQL_HOME 1>&2
) else (
    set "__PATH=%ProgramFiles%"
    for /f "delims=" %%f in ('dir /ad /b "!__PATH!\mysql*" 2^>NUL ^| findstr /i /v "Conn Work" 2^>NUL') do set "_MYSQL_HOME=!__PATH!\%%f"
    if not defined _MYSQL_HOME (
        set __PATH=C:\opt
        for /f "delims=" %%f in ('dir /ad /b "!__PATH!\mysql*" 2^>NUL ^| findstr /i /v "Conn Work" 2^>NUL') do set "_MYSQL_HOME=!__PATH!\%%f"
    )
)
if not exist "%_MYSQL_HOME%\bin\mysql.exe" (
    echo %_ERROR_LABEL% mysql executable not found ^(%_MYSQL_HOME%^) 1>&2
    set _MYSQL_HOME=
    set _EXITCODE=1
    goto :eof
)
set "_MYSQL_PATH=;%_MYSQL_HOME%\bin"
goto :eof
...
set __WHERE_ARGS=
where /q "%MYSQL_HOME%\bin:mysql.exe"
if %ERRORLEVEL%==0 (
    for /f "tokens=1,2,3,*" %%i in ('"%MYSQL_HOME%\bin\mysql.exe" --version 2^>^&1') do set "__VERSIONS_LINE1=%__VERSIONS_LINE1% mysql %%k,"
    set __WHERE_ARGS=%__WHERE_ARGS% "%MYSQL_HOME%\bin:mysql.exe"
)
where /q "%JAVA_HOME%\bin:java.exe"
if %ERRORLEVEL%==0 (
    for /f "tokens=1,2,3,*" %%i in ('call "%JAVA_HOME%\bin\java.exe" -version 2^>^&1 ^| findstr version 2^>^&1') do set "__VERSIONS_LINE1=%__VERSIONS_LINE1% java %%~k,"
    set __WHERE_ARGS=%__WHERE_ARGS% "%JAVA_HOME%\bin:java.exe"
)
set __CJ_JAR=
for /f %%f in ('dir /b /s "%CJ_HOME%\*.jar" 2^>NUL') do set "__CJ_JAR=%%f"
if defined __CJ_JAR (
    for /f "delims=^= tokens=1,*" %%i in ('call "%JAVA_HOME%\bin\java.exe" -cp "%__CJ_JAR%" "%~dp0bin\cj.java" ^| findstr CJ_VERSION 2^>NUL') do set "__VERSIONS_LINE1=%__VERSIONS_LINE1% cj %%~j,"
)
set __CNET_DLL=
for /f %%f in ('dir /b /s "%CNET_HOME%\*.dll" 2^>NUL') do set "__CNET_DLL=%%f"
if defined __CNET_DLL (
    for /f "tokens=1,2,*" %%i in ('call "%~dp0bin\pelook.exe" "%__CNET_DLL%" ^| findstr ProductVersion 2^>NUL') do set "__VERSIONS_LINE1=%__VERSIONS_LINE1% cnet %%~j,"
)
...
if %__VERBOSE%==1 if defined __WHERE_ARGS (
    ...
    if defined CJ_HOME echo    "CJ_HOME=%CJ_HOME%" 1>&2
    if defined CNET_HOME echo    "CNET_HOME=%CNET_HOME%" 1>&2
    if defined GIT_HOME echo    "GIT_HOME=%GIT_HOME%" 1>&2
    if defined JAVA_HOME echo    "JAVA_HOME=%JAVA_HOME%" 1>&2
    if defined MAVEN_HOME echo    "MAVEN_HOME=%MAVEN_HOME%" 1>&2
    if defined MSYS_HOME echo    "MSYS_HOME=%MSYS_HOME%" 1>&2
    if defined MYSQL_HOME echo    "MYSQL_HOME=%MYSQL_HOME%" 1>&2
...
endlocal & (
    if %_EXITCODE%==0 (
        if not defined CJ_HOME set "CJ_HOME=%_CJ_HOME%"
        if not defined CNET_HOME set "CNET_HOME=%_CNET_HOME%"
        if not defined GIT_HOME set "GIT_HOME=%_GIT_HOME%"
        if not defined JAVA_HOME set "JAVA_HOME=%_JAVA_HOME%"
        if not defined MAVEN_HOME set "MAVEN_HOME=%_MAVEN_HOME%"
        if not defined MSYS_HOME set "MSYS_HOME=%_MSYS_HOME%"
        if not defined MYSQL_HOME set "MYSQL_HOME=%_MYSQL_HOME%"
        set "PATH=%PATH%%_MYSQL_PATH%%_GIT_PATH%;%_ROOT_DIR%bin"
